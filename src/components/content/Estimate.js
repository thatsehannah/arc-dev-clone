...
const softwareQuestions = [
  { ...defaultQuestions[0], active: false }, //copying these over will enable us to switch over to the software questions but still have the ability to go back to the first question
  {
    id: 2,
...
const Estimate = (props) => {
  ...
  const handleSelect = (selectedId) => {
    ...
    const newSelected = newQuestions[activeIndex].options[selectedId - 1];
  ...
  const getPlatforms = () => {
    ...
    if (questions.length > 2) {
      questions
        ...
        .map((opt) => selectedPlatforms.push(opt.title));
    }
  ...
  const getFeatures = () => {
    ...
    if (questions.length > 2) {
      questions
        ...
        .map((question) => question.options.filter((option) => option.selected))
        .map((opt) =>
          opt.map((feature) => selectedFeatures.push(feature.title))
        );
    }
  ...
  const websiteSelection = (
    <Grid container direction="column" style={{ marginTop: "14em" }}>
      <Grid item container alignItems="center">
        ...
        <Grid item xs={10}>
          <Typography variant="body1">
            You want {category === "Basic" ? `a ${category}` : `an ${category}`}{" "}
            Website.
          </Typography>
        </Grid>
      </Grid>
            ...
            >
              ...
              <Grid item>
                ...
                >
                  ...
                  question, and we'll get back to you with details moving
